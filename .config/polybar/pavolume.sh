...
autosync='yes'
maxsinks=$(pacmd list-sinks | grep index | cut -d ":" -f2 | awk '{print$1}' | sed -e '$!d')
...
function sinkSwitch {
	if [ ${active_sink} = ${maxsinks} ];
	then
		pacmd list-sink-inputs | grep index | pacmd list-sink-inputs | grep index | awk '{print$2}' | while read -r line; do
			pacmd set-default-sink 0 & pacmd move-sink-input $line 0
		done
		pacmd set-default-sink 0
	else
		pacmd list-sink-inputs | grep index | pacmd list-sink-inputs | grep index | awk '{print$2}' | while read -r line; do
			pacmd set-default-sink `expr $active_sink + 1` & pacmd move-sink-input $line `expr $active_sink + 1` 
		done
		pacmd set-default-sink `expr $active_sink + 1`
	fi
	active_sink=`pacmd list-sinks |awk '/* index:/{print $3}'`
	sink="$(pacmd list-sinks | grep "available: yes" | awk '{print$2}' | sed "`expr $active_sink + 1`q;d")"
